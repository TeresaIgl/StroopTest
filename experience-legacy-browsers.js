...
flowScheduler.add(quitPsychoJS, 'Merci de votre patience.', true);

// quit if user presses Cancel in dialog box:
dialogCancelScheduler.add(quitPsychoJS, 'Merci de votre patience.', false);
...
async function experimentInit() {
  ...
  text_7 = new visual.TextStim({
    ...
    font: 'Arial',
    units: undefined, 
    pos: [(- 0.03), (- 0.1)], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color((-1.0000, -1.0000, -1.0000)),  opacity: undefined,
    depth: -3.0 
  });
...
function pause_testRoutineBegin(snapshot) {
  return async function () {
    ...
    pause_testComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function colorRoutineBegin(snapshot) {
  return async function () {
    ...
    colorComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function random_R_NRRoutineBegin(snapshot) {
  return async function () {
    ...
    random_R_NRComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
